...
function get20s(arrayOfArtists) {
  ...
  for (let i = 0; i < arrayOfArtists.length; i++) {
    // const [born, died] = arrayOfArtists[i].years.split(' - ')
    const born = arrayOfArtists[i].years.split(" - ")[0];
    const died = arrayOfArtists[i].years.split(" - ")[1];
    if (born >= 1900 && died <= 2000) {
      newArray.push(arrayOfArtists[i].name);
    }
  }
  return newArray;
}
...
function artistByCountry(arrayOfArtists, nationality) {
  const newArray = [];
  for (let i = 0; i < arrayOfArtists.length; i++) {
    if (arrayOfArtists[i].nationality === nationality) {
      newArray.push(arrayOfArtists[i].name);
    }
  }
  return newArray;
